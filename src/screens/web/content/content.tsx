...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Modal, KeyboardAvoidingView, Platform, Share, Image, Linking, Alert, ActivityIndicator, BackHandler } from 'react-native'
...
import { GET_COMMENT_DETAILS_BY_CONTENT_ID_MOBILE, GET_CONTENT_DETAILS_BY_ID_MOBILE, GET_HUBS_MOBILE, GET_RELATED_CONTENT_DETAILS_BY_ID_MOBILE, GET_SPACE_CONTENT_ID, GET_SPACE_INFO_BY_ID, GET_SPACES_MOBILE, GET_STAFF_DIRECTORY, GET_SUBSCRIBED_SPACES_HUB_MOBILE } from '@/src/services/QueryMethod';
import Search from '@/src/components/search';
import { POST_COMMENT, REQUEST_SPACE_ACCESS, TOGGLE_COMMENT_LIKE, TOGGLE_CONTENT_FAV, TOGGLE_CONTENT_LIKE } from '@/src/services/MutationMethod';
import { useIsFocused } from '@react-navigation/native';
import { useDispatch, useSelector } from 'react-redux';
import { setHub, setSpace, setVideoTime } from '@/src/redux/action';
...
const Content: React.FC = (props: any) => {
    ...
    const spaceDashboard = useSelector((state: any) => state.reducer.spaceDashboard)
    ...
    const [isLoadingSD, setIsLoadingSD] = useState(true);
    ...
    const arrangeChapter = (data: any) => {
        console.log("CONTENTCHAPTERS", data)
        let chapters = []
        for (let key in data) {
            let chapter = {
                id: data[key]?.id,
    ...
    //         y: config.getHeight(80), // Vertical scroll position (in pixels)
    //         animated: true,
    //     });
    ...
    const toggleLikeContentRelated = async (contentIdRelated: number) => {
        console.log('toggleLikeContentRelatedtoggleLikeContentRelated' + contentIdRelated);
        const responseToggleLiked = await requestToggleLike({
            variables: {
                input: {
    ...
    const refetchContentAPI = async () => {
        ...
        if (responseRefetchContentAPI?.data?.getContentInfoById?.success) {
            ...
            if (content_type_id === ContentType.VIDEOS
                || content_type_id === ContentType.PODCASTS
            ) {
            ...
            // alert(contentData?.getContentInfoById?.contentInfo?.is_liked)
    ...
    const MainContent = (
        <View style={{
            flex: 1, justifyContent: 'flex-start', alignItems: 'center', width: getWidth(50),
        ...
        }}>
            {
                isLoading ? (
                    ...
                    <>
                        {/* <ContentTabBar type={contentdetails?.title !== "Directory" ? contentData?.getContentInfoById?.contentInfo?.content_type_info?.name : "Directory"} /> */}
                        <ContentTabBar type={contentdetails?.content_type_id === ContentType.DIRECTORY ? contentdetails?.content_type_id : content?.content_type_info?.id} />
                            ...
                            >
                                <ContentHeader title={contentdetails?.content_type_id === ContentType.DIRECTORY ? "Staff Directory" : content?.content_title} />
                                {
                                    (contentdetails !== null && contentdetails?.content_type_id === ContentType.DIRECTORY)
                                        ...
                                        <View style={styles.searchContainer}>
                                            <SearchBar isOnKeyboardSearch={true} onSearch={onSearch} onChangeSearch={onSearch} containerWidth={config.isWeb ? getWidth(140) : config.getWidth(95)} borderColor={borderColors.signInBorder}
                                            />
                                        </View>
                                        : null
                                ...
                                {/* {(contentTypeId === ContentType.ARTICLES || contentTypeId === ContentType.STORYCASTS) && (
                                    <OpenArticleLinkButton
                                        ...
                                        url={contentTypeId === ContentType.ARTICLES ? articleData?.url : storyCastData?.url}
                                        textStyle={{ color: '#000' }}
                                    />
                                )} */}
...

                        {
                            ...
                            >
                                <View style={{
                                    ...
                                    borderRadius: getWidth(2), borderWidth: 1, borderColor: borderColors.profileImage, backgroundColor: commonColors.white
                                }}>
                                    ...
                                    <TouchableOpacity onPress={() => {
                                        ...
                                        <Image
                                            style={{
                                                width: getWidth(5),
                                                height: getWidth(5), alignSelf: 'flex-end', margin: getWidth(2)
                                            }}
                                            source={Icons.cross}
                                            resizeMode='contain'
                                    ...
                                    <ScrollView style={{
...

                                        {
                                            ...
                                            }} style={{ margin: getWidth(2), flexDirection: 'row', justifyContent: 'space-between' }}>
                                                <View>
                                                    <Text style={{ fontFamily: 'bold', fontSize: config.isWeb ? getFontSize(8) : config.generateFontSizeNew(16), color: commonColors.black }}>
                                                        Email:
                                                    </Text>
                                                    <Text style={{ fontFamily: 'regular', fontSize: config.isWeb ? getFontSize(8) : config.generateFontSizeNew(16), color: commonColors.black }}>
                                                        {StaffDirectoryUser?.email}
                                                    </Text>
                                                </View>

                                                {
                                                    StaffDirectoryUser?.preferred_contact_method === 0
                                                        ...
                                                        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                                            <Image
                                                                source={Icons.star}
                                                                style={{ width: getWidth(4), height: getWidth(4) }}
                                                                resizeMode='contain'
                                                            />
                                                        </View>
                                                        : null
                                        ...
                                        }
                                        {
                                            ...
                                            <TouchableOpacity onPress={() => {
                                                openDialer(StaffDirectoryUser?.cell_phone)
                                            }} style={{ margin: getWidth(2) }}>
                                                <View>
                                                    <Text style={{ fontFamily: 'bold', fontSize: config.isWeb ? getFontSize(8) : config.generateFontSizeNew(16), color: commonColors.black }}>
                                                        Cell phone:
                                                    </Text>
                                        ...
                                        }
                                        {
                                            ...
                                            <TouchableOpacity onPress={() => {
                                                openDialer(StaffDirectoryUser?.pager)
                                            }} style={{ margin: getWidth(2) }}>
                                                <View>
                                        ...
                                        }
                                        {
                                            ...
                                            <TouchableOpacity onPress={() => {
                                                openDialer(StaffDirectoryUser?.phone)
                                            }} style={{ margin: getWidth(2) }}>
                                                <View>
                                                    <Text style={{ fontFamily: 'bold', fontSize: config.isWeb ? getFontSize(8) : config.generateFontSizeNew(16), color: commonColors.black }}>
                                                        Phone:
    ...
    return (
        <ScrollView ref={scrollViewRef} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center' }} style={{ paddingBottom: config.isWeb ? getHeight(5) : null, height: getViewHeight(100) }}>
