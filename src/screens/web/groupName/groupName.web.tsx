...
import { View, Text, StyleSheet, Image, ActivityIndicator, Alert, TouchableOpacity, SectionList, TextInput, ScrollView, Modal, FlatList } from 'react-native';
...
const GroupNameWeb: React.FC = ({ route }) => {
    ...
    const { loading, error, data, refetch } = useQuery(GET_GROUP_DIRECTORY_DATA, {
        ...
        onError: (error) => {
            ...
            alert('Error', 'Failed to load group data. Please try again.');
        },
        onCompleted: (data) => {
    ...
    const loadMoreData = async () => {
        ...
        try {
            ...
            alert('Error', 'Failed to load more users. Please try again.');
        } finally {
    ...
    if (error && !allUsers.length) {
        return (
            <View style={[styles.container, styles.centerContent]}>
                <Text style={[styles.errorText, { fontSize: config.isWeb ? getFontSize(3) : config.generateFontSizeNew(14), }]}>Failed to load group data</Text>
        ...
        <View style={[styles.subcontainer, { width: getWidth(60), }]}>
            <View style={styles.header}>
                {/* {searchVisible ? (
                    <TextInput
                        ...
                        style={[styles.searchBar, { fontSize: config.isWeb ? getFontSize(3) : config.generateFontSizeNew(14), }]}
                    ...
                    <Text style={[styles.allGroupsText,{
                ...
                <View style={styles.iconsContainer}>
                    <TouchableOpacity onPress={() => setSearchVisible(!searchVisible)}>
                        <Image source={Icons.search} resizeMode='contain' style={[styles.icon, { marginLeft: config.isWeb ? getWidth(0) : config.getWidth(3), }]} />
                    </TouchableOpacity>
                    <TouchableOpacity ref={threeDotsRef} onPress={toggleMenu}>
                        <Image source={Icons.threeDots} resizeMode="contain" style={[styles.icon, { marginLeft: config.isWeb ? getWidth(0) : config.getWidth(3), }]} />
                    </TouchableOpacity>
                        ...
                        <TouchableOpacity style={styles.modalOverlay} onPress={toggleMenu}>
                            <View style={[
                                styles.menuContainer,
                                {
                                    ...
                                    padding: config.isWeb ? getWidth(0.5) : config.getWidth(3),
                                    width: config.isWeb ? getWidth(10) : config.getWidth(45),
                                }
                            ]}>
                                <FlatList
                                    data={menuItems}
            ...
            <SectionList
                ...
                renderSectionHeader={({ section: { title } }) => (
                    <Text style={[styles.sectionHeader, { fontSize: config.isWeb ? getFontSize(4) : config.generateFontSizeNew(16), }]}>{title}</Text>
                )}
                renderItem={({ item }) => (
                    <View style={[styles.userItem, { paddingVertical: config.isWeb ? getHeight(2) : config.getHeight(0.5), }]}>
                        <Image
                            source={item.image ? { uri: item.image } : Icons.userProfile}
                            resizeMode='cover'
                            style={[styles.userImage, {
                                width: config.isWeb ? getWidth(3.5) : config.getWidth(13),
                                height: config.isWeb ? getWidth(3.5) : config.getWidth(13),
                            }]}
                        />
                        <View>
                            <Text style={[styles.userName, { fontSize: config.isWeb ? getFontSize(4) : config.generateFontSizeNew(16), }]}>{item.name}</Text>
                            <Text style={[styles.userSpeciality, { fontSize: config.isWeb ? getFontSize(2.5) : config.generateFontSizeNew(11), }]}>{item.speciality}</Text>
                        </View>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    userImage: {
        ...
        marginRight: config.getWidth(2.5),
        borderColor: borderColors.profileImage,
        borderWidth: 1.5
    },
    userName: {
