import { View, Text, Image, KeyboardAvoidingView, TextInput, TouchableOpacity, FlatList, StyleSheet, ActivityIndicator, ScrollView, TouchableWithoutFeedback, Modal, Platform, Keyboard, Alert } from 'react-native'
...
import { setAffiliations, setCurrentTab, setSocieties, setSpecialities } from '@/src/redux/action'
...
const SpaceCreationWeb: React.FC = () => {
    ...
    const getParentHeight = (height: number) => {
    ...
    const requestSpace = async () => {
        ...
        console.log('affiliationIdaffiliationId' + affiliationId);
        ...
        if (responseRequestSpace?.data?.requestSpaceCreation?.success) {
            ...
            setTimeout(() => {
                setShowDialog(false);
                setTimeout(() => {
                    // navigationService.reset([{ name: RouteNames.HomeNavigator }]);
    ...
    const Item = ({ title, id, type }: ItemProps) => (
        <TouchableOpacity onPress={() => {
            ...
            <Text style={{
                ...
                fontSize: config.isWeb ? getFontSize(3.2) : config.generateFontSizeNew(10),
    ...
    const handle = (value: string, valueId: number) => {
    ...
    const MainContent = (
        <KeyboardAvoidingView
            style={{ flex: 1, width: getWidth(60) }}
                ...
                <TouchableWithoutFeedback disabled={!showDialog}>
                    <ScrollView scrollEnabled={!showDialog}>
                        <Text style={[styles.headingText, {
                            marginLeft: config.isWeb ? getWidth(14) : config.getWidth(5),
                            marginRight: config.isWeb ? getWidth(14) : config.getWidth(5),
                            fontSize: config.isWeb ? getFontSize(4) : config.generateFontSizeNew(12),
                        }]}>
                        ...
                        <View style={{ justifyContent: 'center', alignItems: 'center', marginBottom: config.getHeight(5) }}>
                            ...
                            {showDialog === false ? <TouchableOpacity onPress={() => { requestSpace() }}>
                                <View style={[styles.buttonStyle, {
                                    paddingVertical: config.isWeb ? getHeight(1) : config.getHeight(4),
                                    paddingHorizontal: config.isWeb ? getWidth(6) : config.getWidth(35),
                                }]}>
    ...
    return (
        ...
        <ScrollView
            ...
            style={{ paddingBottom: config.isWeb ? getHeight(5) : null, height: getParentHeight(100), }}
        >
            <WebBaseLayout
...
const styles = StyleSheet.create({
    ...
    alertTitle: {
        fontSize: config.generateFontSizeNew(5),
        fontWeight: 'bold',
        color: commonColors.black,
        marginBottom: 10,
        textAlign: 'center',
    ...
    alertMessage: {
        fontSize: config.generateFontSizeNew(4),
        color: commonColors.black,
        fontFamily: 'regular',
        marginBottom: 20,
        textAlign: 'center',
...
export default SpaceCreationWeb
