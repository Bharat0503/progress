...
import { setCollection, setCollectionList, setContent, setContentId, setContentIdList, setRefresh, setSpaceDashBoard, setStartfromSpaceDashBoard } from '@/src/redux/action';
...
const Collections: React.FC = (props: any) => {
    ...
    //     const unsubscribe = navigationService.addListener('blur', (e) => {
    //         e.preventDefault();
    //         handleBackPress();
    //     });
    ...
    //                 // window.history.pushState(null, '', window.location.pathname); // Push same state to prevent back
            ...
            {isLoading ? (
                ...
                <ScrollView>
                    ...
                    <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ fontFamily: 'regular', fontSize: config.isWeb ? getFontSize(10) : config.generateFontSizeNew(18), textAlign: 'center' }}>{collection?.name}</Text>
                        <Text style={{ fontFamily: 'regular', fontSize: config.isWeb ? getFontSize(12) : config.generateFontSizeNew(30), textAlign: 'center', fontWeight: '600' }}>{space?.name} Collections</Text>
                    </View>

                    {
                        subCollection?.length !== 0 &&
                        <View style={{
                            width: config.isWeb ? getWidth(120) : config.getWidth(90),
                            ...
                            backgroundColor: commonColors.white,
                            borderRadius: config.isWeb ? getWidth(3) : config.getWidth(3),
                            borderWidth: 1,
                            ...
                            <TouchableOpacity onPress={() => {
                                ...
                                <Text style={{
                                    fontFamily: 'bold', fontSize: config.isWeb ? getFontSize(7) : config.generateFontSizeNew(14)
                                }}>
                                    Sub Collections
                                </Text>
                            </TouchableOpacity>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View style={{
                                ...
                                marginTop: config.isWeb ? config.getHeight(1) : config.getHeight(2), overflow: 'hidden'
                            }}>
                                <Text style={{
                                    color: item?.color, fontFamily: 'regular',
                                    fontSize: config.isWeb ? getFontSize(4) : config.generateFontSizeNew(16),
                                    marginLeft: config.isWeb ? getWidth(0.5) : config.getWidth(2),
                                    marginTop: config.getWidth(2),
                                }}>
                                    {
                                        item?.name
                                    }
