...
import { View, Text, StyleSheet, TouchableOpacity, Platform, Dimensions, ScrollView, Alert, ActivityIndicator, FlatList, Linking, } from 'react-native'
...
import { setContent, setContentId, setCurrentTab, setDeepLinkHandledRef, setSpace, setSpaceDashBoard, setSpeciality, setStartfromSpaceDashBoard, setTempToken, setTempUserId, setToken, setUserId } from '@/src/redux/action';
...
import { GET_LIBRARY_FAVORITES_DATA, GET_SUBSCRIBED_SPACES_HUB_FILTER_SEARCH_MOBILE, GET_SUBSCRIBED_SPACES_HUB_HOME, GET_SUBSCRIBED_SPACES_HUB_MOBILE, GET_USER_DETAILS } from '@/src/services/QueryMethod';
...
const Home: React.FC = (props) => {
    ...
    const deepLinkHandledRef = useSelector((state: any) => state.reducer.setDeepLinkHandledRef)
    const PAGE_SIZE = 10;
    ...
    useEffect(() => {
        if (!loading) {
            dispatch(setSpeciality(data?.getUserDetails?.userInfo?.speciality_info?.name !== "Others" ?
                data?.getUserDetails?.userInfo?.speciality_info?.name :
                data?.getUserDetails?.userInfo?.custom_info?.custom_speciality))
        }
    ...
    useEffect(() => {
            ...
            if (!isNaN(contentId) && contentId !== 0) {
                ...
                if (token) {
                    ...
                    if (event.url.includes(targetDomain)) {
                        if (!deepLinkHandledRef) {
                            // Mark the deep link as handled
                            dispatch(setDeepLinkHandledRef(true));
        ...
        const checkInitialURL = async () => {
            ...
            if (initialUrl && !deepLinkHandledRef) {
                console.log('Handling initial deep link:', initialUrl);
                handleDeepLink({ url: initialUrl });
            }
        };

        // Check the initial URL when the app starts
        checkInitialURL();

        // Add a listener for subsequent deep links
    ...
    const arrangeDashboardData = (data: any) => {
        ...
        //     color: space.color,
        // }));
        //console.log('Featured Items-->', spaces);
    ...
    const renderItem = ({ item }: { item: any }) => (
        <TouchableOpacity onPress={() => handleFeatureClick(item)} style={{}}>
    ...
    // const getDownloadedFiles = async () => {
    //     try {
    //         let fileSystemFiles: any[] = [];

    //         if (Platform.OS === 'web') {
    //             const storedFiles = localStorage?.getItem('downloadedFiles');
    //             fileSystemFiles = storedFiles ? JSON.parse(storedFiles) : [];
    //         } else {
    //             fileSystemFiles = await FileDownloadService?.getAllDownloadedFiles();
    //             const asyncFiles = await AsyncStorage?.getItem('downloadedFiles');
    //             const storedFiles = asyncFiles ? JSON.parse(asyncFiles) : [];
    //             fileSystemFiles = storedFiles?.filter((asyncFile: { fileId: string }) =>
    //                 fileSystemFiles?.some(file => file?.id === asyncFile?.fileId)
    //             );
    //         }
    //         console.log('Filtered Downloaded Files:', fileSystemFiles);
    ...
    return (
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center' }} style={{ paddingBottom: config.isWeb ? getHeight(5) : null, height: getHeight(100) }}>
            <WebBaseLayout showSearch onSearch={() => { '' }} searchText={searchText} leftContent={
                <>
                    <MostSearchedTerms />
                    {CreateGroup}
                </>
            } onSearchBoxClick={() => onSearch('')} >
                <View style={[styles.container, { width: getWidth(50) }]}>
                    ...
                    </View>
                    {
                        ...
                        < View style={styles.channelList}>
                            {/* <ChannelContainer subscribedSpaces={subscribedSpaces} /> */}
                    ...
                    }
                    {
                        ...
                        <>
                            ...
                            {dashBoardDataLoading ? (
                                ...
                                <View style={{ width: config.isWeb ? getWidth(50) : null, justifyContent: 'center', alignItems: 'center' }}>
...
const styles = StyleSheet.create({
    ...
    mostSearchContainer: {
        justifyContent: 'flex-start', alignItems: 'center', marginTop: 20, height: config.getHeight(30)
    },
    expandContainer: {
