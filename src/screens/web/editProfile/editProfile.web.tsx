...
import { View, Text, StyleSheet, TouchableOpacity, Image, ScrollView, ActivityIndicator, Modal, Alert, Platform } from 'react-native'
...
const EditProfile: React.FC = (props) => {
    ...
    //             if (result) {
    //                 const imageUrl = URL.createObjectURL(result); // Temporary local URL for the preview
    //                 setProfileImage(imageUrl);
    //                 let formData = new FormData()
    //                 // const profile_Image = {
    //                 //     uri: imageUrl, // File URI
    //                 //     mimetype: 'application/octet-stream',
    //                 //     filename: `${Date.now()}-${Math.floor(Math.random() * 1e6)}`,
    //                 //     fieldName: '0',
    //                 //     encoding: '7bit',
    //                 // }
    ...
    //                 console.log("result", blob, token)
    //                 // console.log("result", formData?._parts[0]?.profile_image, token)
    //                 try {
    ...
    //                         }
    //                     );
    //                     console.log("uploadProfileImageuploadProfileImageuploadProfileImage", response)
    //                 }
    //                 catch (error) {
    ...
    //                 //     variables: { profileImage: profile_Image },
    //                 // });
    //                 // console.log("uploadProfileImage", profile_Image, response)
    //             }
    ...
    //     //             file: {
    //     //                 uri: uri, // File URI
    //     //                 filename: `${Date.now()}-${Math.floor(Math.random() * 1e6)}.jpg`,
    ...
    //     //     filename: `${Date.now()}-${Math.floor(Math.random() * 1e6)}`
    //     // }
    //     // console.log("uploadProfileImage", profile_Image)
    //     // const response = await uploadImage({
    //     //     variables: { profileImage: profile_Image },
    //     // });


    //     // const response = await fetch('https://7531-2405-201-4056-306c-30af-5e22-e03a-44f4.ngrok-free.app/graphql', {
    //     //     method: 'POST',
    //     //     headers: {
    //     //         Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI4NWE3ODc4MC0zOGIyLTQwZDEtOWZiZC0yZDFiOTI0NmQxOTYiLCJlbWFpbCI6ImJoYXJhdGFkdmFuaTk3QGdtYWlsLmNvbSIsImlhdCI6MTczMzg1MDkyMywiZXhwIjoxNzM2NDQyOTIzfQ.MDRKNpM4tQYzBmeQr06WQ6iKSjBo81lMig3AWxp10iI', // Optional if authentication is required
    //     //     },
    //     //     body: formData,
    //     // });
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled) {
          ...
          const formData = new FormData();
          formData.append('profile_image', blob, result.assets[0]?.fileName as any);
          try {
            const response = await fetch(
              `${BASE_API_URL_PROD}/uploadProfileImage`,
              {
            ...
            console.log('uploadProfileImageWeb', response);
          } catch (error) {
            console.log('uploadProfileImageError', error);
          }
        }
...

            {
                loading
                    ...
                    <ScrollView contentContainerStyle={{
                        ...
                        width: getWidth(100), justifyContent: 'center', alignItems: 'center',
                    }}>
                        <View style={{
                            ...
                            <View style={{
                                ...
                                <View style={{ flex: 0.5, borderRightWidth: getHeight(0.1), borderRightColor: borderColors.profileImage, paddingVertical: getHeight(10), alignItems: 'flex-end', paddingRight: getWidth(5), }}>
                                    {
                                        profileImage
                                            ?
                                    ...
                                    }} style={{ backgroundColor: backgroundColors.editProfile, width: getWidth(12), height: getHeight(5.5), marginRight: getWidth(2), marginTop: getHeight(2), borderRadius: getWidth(1), borderWidth: 1, borderColor: borderColors.profileImage, justifyContent: 'center', alignItems: 'center' }}>
                                ...
                                <View style={{
                                    flex: 0.5, borderLeftWidth: getHeight(0.1), borderLeftColor: borderColors.profileImage, paddingVertical: getHeight(2), alignItems: 'flex-start', paddingLeft: getWidth(5),
                                    ...
                                    <View style={{
                                        ...
                                        <View style={{
                                            ...
                                            {firstNameError &&
                                                <Text
                                                    style={{
                                                        ...
                                                        alignSelf: 'flex-start', marginLeft: getWidth(0.5),
                                            ...
                                            {
                                                phoneNumber != null ?
                                                    <TitleInput title={"Phone Number"} notImp initialData={phoneNumber} editable={false} setInput={handlePhone} />
                                                    : null
                                            }
                                    ...
                                    <TouchableOpacity disabled={dataLoader} onPress={() => {
                                        ...
                                        // navigationService.navigate(RouteNames.EditProfile)
                                    }} style={{ backgroundColor: backgroundColors.editProfile, width: getWidth(12), height: getHeight(4), marginRight: getWidth(2), marginTop: getHeight(2), borderRadius: getWidth(1), borderWidth: 1, borderColor: borderColors.profileImage, justifyContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
                                        <Text style={{ color: commonColors.white, fontFamily: 'bold', fontSize: getFontSize(3) }}>
                                            Save
                                        </Text>
                                    </TouchableOpacity>
...
//                 <TouchableOpacity onPress={() => {
//                     pickImage()
//                 }} style={{ width: config.isWeb ? getWidth(10) : config.getWidth(20), marginTop: config.getHeight(2) }}>
...
//                                         borderRadius: config.isWeb ? getWidth(5) : config.getWidth(10), 
//                                         borderWidth: 1, borderColor: borderColors.profileImage, marginTop: config.getHeight(2) }}
//                                     source={{ uri: profileImage }}
//                                     resizeMode='stretch'
//                                     onLoadStart={handleLoadStart}
//                                     onLoadEnd={handleLoadEnd}
...
//                 <View style={{ width: config.isWeb ? getWidth(40) : config.getWidth(90), marginTop: config.getHeight(3), alignItems: 'flex-end', justifyContent: 'center', }}>
//                     <TouchableOpacity disabled={dataLoader} onPress={() => {
//                         let flag = true
//                         if (!firstName) {
//                             setFirstNameError("Please provide the first name")
...
//                     }} style={{ backgroundColor: backgroundColors.saveButton, width: config.isWeb ? getWidth(10) : config.getWidth(30), height: config.getHeight(5), borderRadius: config.getWidth(4.5), borderWidth: 1, borderColor: borderColors.profileImage, justifyContent: 'center', alignItems: 'center' }}>
//                         <Text style={{
//                             fontFamily: 'regular',
